...
for method in args.method_list:
    ...
    for prune_amount in args.ratio_list:
        ...
        net = getattr(__import__("torchvision.models", fromlist=[""]), args.model)().to(
            device
        )

        # Apply prune
        input_shape = list(trainloader.dataset.__getitem__(0)[0][:].shape)
        pruner = getattr(__import__("pruning_method." + method, fromlist=[""]), method)(
